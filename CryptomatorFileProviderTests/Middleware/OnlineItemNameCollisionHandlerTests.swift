...
class OnlineItemNameCollisionHandlerTests: XCTestCase {
	...
	override func setUpWithError() throws {
		...
		metadataManager = MetadataDBManager(with: dbPool)

		middleware = OnlineItemNameCollisionHandler(itemMetadataManager: metadataManager)
	}
	...
	func testHandleCollision() throws {
		...
		let itemMetadata = ItemMetadata(name: "foo.txt", type: .file, size: nil, parentId: MetadataDBManager.rootContainerId, lastModifiedDate: nil, statusCode: .isUploading, cloudPath: originalCloudPath, isPlaceholderItem: true)
		...
		middleware.execute(task: SampleCloudTask(itemMetadata: itemMetadata)).then {
			guard let itemID = itemMetadata.id, let cachedItemMetadata = try self.metadataManager.getCachedMetadata(for: itemID) else {
			...
			XCTAssertEqual(MetadataDBManager.rootContainerId, cachedItemMetadata.parentId)
	...
	func testRetryOnlyOnce() throws {
		...
		let itemMetadata = ItemMetadata(name: "foo.txt", type: .file, size: nil, parentId: MetadataDBManager.rootContainerId, lastModifiedDate: nil, statusCode: .isUploading, cloudPath: CloudPath("/foo.txt"), isPlaceholderItem: true)
		try metadataManager.cacheMetadata(itemMetadata)
		middleware.setNext(AnyWorkflowMiddleware(workflowMock))
		middleware.execute(task: SampleCloudTask(itemMetadata: itemMetadata)).then {

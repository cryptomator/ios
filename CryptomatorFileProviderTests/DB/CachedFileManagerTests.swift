...
class CachedFileManagerTests: CacheTestCase {
	...
	func testRemoveCachedFile() throws {
		...
		let metadata = ItemMetadata(id: 2, name: "Foo", type: .file, size: nil, parentID: NSFileProviderItemIdentifier.rootContainerDatabaseValue, lastModifiedDate: nil, statusCode: .isUploaded, isPlaceholderItem: false)
	...
	func testRemoveCachedFileForPendingUpload() throws {
		...
		let uploadManager = UploadTaskDBManager(database: inMemoryDB, itemMetadataManager: ItemMetadataDBManager(database: inMemoryDB))
	...
	func testGetLocalCacheSizeInBytes() throws {
		...
		let metadata = urls.enumerated().map { idx, url in ItemMetadata(id: Int64(idx + 2), name: url.lastPathComponent, type: .file, size: nil, parentID: NSFileProviderItemIdentifier.rootContainerDatabaseValue, lastModifiedDate: nil, statusCode: .isUploaded, isPlaceholderItem: false) }
		let data = getRandomData(sizeInBytes: 256)

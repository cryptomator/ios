...
class MaintenanceManagerTests: XCTestCase {
	...
	func testPreventCreatingDownloadTasks() throws {
		...
		// Prevent INSERT
		try manager.enableMaintenanceMode()
		try checkThrowsMaintenanceError(downloadTaskManager.createTask(for: itemMetadata, replaceExisting: true, localURL: XCTUnwrap(URL(string: "/Test")), onURLSessionTaskCreation: nil))
	}
